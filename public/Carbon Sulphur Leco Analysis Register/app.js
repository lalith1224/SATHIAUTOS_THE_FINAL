document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                const response = await fetch('http://localhost:3000/api/carbon-sulphur-leco-analysis-register', {
                    method: 'POST',
                    headers: { 
                ...
                if (response.ok) {
                    ...
                    showAlert('success', result.message || 'Carbon Sulphur analysis submitted successfully!');
                    form.reset();
    ...
    function populateTable(results) {
        ...
        results.forEach(result => {
            ...
            row.innerHTML = `
                ...
                <td>${result.identification_data_heat_code || ''}</td>
                <td>${result.leco_c_percent || ''}</td>
                <td>${result.leco_s_percent || ''}</td>
                <td>${result.spectro_c_percent || ''}</td>
                <td>${result.spectro_s_percent || ''}</td>
            ...
            tableBody.innerHTML = `
                <tr>
                    <td colspan="11" style="text-align: center; padding: 2rem; color: #64748b;">
                        No carbon sulphur analysis records found.
                    </td>
                </tr>
            `;
        }
    ...
    function showTableError(message) {
        ...
        if (tableBody) {
            tableBody.innerHTML = `
                <tr>
                    <td colspan="11" style="text-align: center; padding: 2rem; color: #dc2626;">
                        ${message}
                    </td>
                </tr>
            `;
